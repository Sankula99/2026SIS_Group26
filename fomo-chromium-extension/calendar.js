const FomoCalendar = (() => {
    ...
    function parseEventProperty(line) {
        let colon = -1;
        let quoted = false;
        ...
        for (const part of parameterParts) {
            ...
            (parameters[key] ??= []).push(...value.split(","));
        }
        const value = line.slice(colon + 1);
        return {
            decodedValue: value.replace(/\\([nN,;\\])/g, (_, escaped) =>
                /[nN]/.test(escaped) ? "\n" : escaped,
            ), // replace funny characters used by iCal with readable version
            parameters,
        };
    }
    ...
    function parseCalendar(text) {
        ...
        function addActivity(value) {
            ...
            if (fields.length !== 3 || fields.some((field) => !field.trim()))
                return;
            const [subject, activity, number] = fields.map((field) =>
                field.trim(),
            );
            if (
                !/^\d+(?:_[A-Za-z0-9]+){4}$/.test(subject) || // Validate subject, activity and number structure loosely
                !/^[A-Za-z][A-Za-z0-9]*$/.test(activity) ||
                !/^\d+$/.test(number)
            )
                return;
        ...
        for (const line of lines) {
            if (line.startsWith("BEGIN:")) {
                ...
                if (components.pop() !== component) {
                    throw new Error(
                        "The ICS calendar contains an incomplete component.",
                    );
                }
                if (component === "VEVENT") {
                    ...
                    events.push({
                        ...
                        // Keyed by ICS property name; values are arrays to retain repeats.
                        // This is the structured source for mapping to the data dictionary.
                        properties,
                    });
                }
            } else if (components.at(-1) === "VEVENT") {
                const property = parseEventProperty(line);
                if (property) {
                    const name = line
                        .slice(0, line.search(/[;:]/))
                        .toUpperCase();
                    (properties[name] ??= []).push(property);
                }
        ...
        if (components.length)
            throw new Error("The ICS calendar is incomplete.");
        if (eventCount && !seen.size) {
            throw new Error(
                "No timetable activities were found in the calendar descriptions.",
            );
        }
